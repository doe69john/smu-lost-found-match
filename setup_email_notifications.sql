-- Setup Email Notifications for Match Alerts
-- This adds automatic email notifications when matches are found

-- 1. Create function to invoke the notification Edge Function
...
-- 2. Create trigger function that fires after match insert
...
BEGIN
  ...
  IF lost_item_status = 'completed' AND TG_OP = 'INSERT' THEN
    ...
    IF match_count = 1 THEN
      -- Wait briefly for other matches to be inserted (same batch)
      PERFORM pg_sleep(2);
...
-- 3. Create trigger on matches INSERT
...
DO $$
BEGIN
  RAISE NOTICE '✅ Email notification system configured successfully!';
  RAISE NOTICE 'Users will now receive emails when matches are found for their lost items.';
END $$;
