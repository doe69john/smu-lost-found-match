...
const onSubmit = async (values, { resetForm }) => {
  ...
  try {
    ...
    if (selectedFile.value) {
      ...
      const storagePath = `${user.value.id}/found/${Date.now()}.${extension}`
    ...
    pushToast({
      ...
      message: 'Thank you for helping reunite items with their owners.',
      variant: 'success'
    })
...
<template>
  <section class="d-grid gap-4">
    <header>
      ...
      <p class="text-muted mb-0">Share details so we can notify students who may be missing this item.</p>
    </header>
    ...
    <VForm :validation-schema="validationSchema" :initial-values="initialValues" @submit="onSubmit">
      <template #default>
        <div class="row g-3">
          ...
          <div class="col-12 col-md-6">
            <label class="form-label" for="found-model">Model</label>
          ...
          <div class="col-12 col-md-6">
            <label class="form-label" for="found-color">Colour</label>
          ...
          <div class="col-12">
            ...
            <Field name="description" v-slot="{ field, errorMessage }">
              <textarea
                ...
                class="form-control"
                :class="{ 'is-invalid': errorMessage }"
                placeholder="Describe the item and its condition so we can match it accurately."
                v-bind="field"
              ></textarea>
              <div v-if="errorMessage" class="invalid-feedback d-block">{{ errorMessage }}</div>
            </Field>
          ...
          <div class="col-12 col-md-6">
            <label class="form-label" for="found-location">Where did you find it? *</label>
            <Field name="location_found" v-slot="{ field, errorMessage }">
              <input
                ...
                class="form-control"
                :class="{ 'is-invalid': errorMessage }"
                placeholder="e.g. School of Accountancy Atrium"
                v-bind="field"
              />
              <div v-if="errorMessage" class="invalid-feedback d-block">{{ errorMessage }}</div>
            </Field>
          ...
          <div class="col-12 col-md-3">
            <label class="form-label" for="found-time">Approximate time</label>
          ...
          <div class="col-12">
            <label class="form-label" for="found-unique">Unique features</label>
          ...
          <div class="col-12">
            <label class="form-label" for="found-photo">Upload a photo (optional)</label>
            <input id="found-photo" type="file" accept="image/*" class="form-control" @change="onFileChange" />
            <small class="text-muted">Include a clear photo if possible.</small>
